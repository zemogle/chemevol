...
class ChemModel:
    def __init__(self, **inputs):
        ...
        try:
            ...
            self.inflow_metalfractions = inputs['Pristine_isotope_fractions']
    ...
    def gas_metal_dust_mass(self, sn_rate):
            ...
            metals = self.Z_init*mg*np.array(self.inflow_metalfractions)
            ...
            for item, t in enumerate(time[(time < self.tend) & (time>self.tstart)]):
                ...
                if gas_out*0.03>0.5*mg:
                    logger.warning('SFR has been reduced by factor %s at time= %s Gyr because more than 50%% of the gas was being blown out in a single 30 Myr timestep for model %s'%((gas_out*0.03)/(0.5*mg),t,self.name))
                    SFR=SFR*0.5*mg/(gas_out*0.03)
                    gas_inf,gas_out,outflows = gas_inandout(
                        redshift,\
                        self.inflows['on'],\
                        self.outflows['on'],\
                ...
                if self.recycle['on']:
                    recycle_gas,recycle_dust,recycle_Z = \
                        recycle(t, dt, redshift, mstars, time, recycle_gas, recycle_dust, recycle_Z, outflows, gas_out, mdust_out, np.asarray(metals_out), self.recycle['esc_prob_perGyr'], self.recycle['reaccr_time_factor'], nisotopes)
