...
def runModel(item):
    ...
    ascii.write(params,"Models/"+str(name+'.dat'),names=paramsorder)
...
def main():
    ...
    try:
        data = np.genfromtxt("modelGrid_dust_best.csv", dtype=alttype,delimiter=',', autostrip=True, names=names)
    except ValueError:
    ...
    for i in range(0,len(data)):
        if not os.path.exists("./Models/"+data[i][0]+".dat"):
		gal_tup = zip(names, data[i])
        	...
        	gal_data['outflows'] = {'on': gal_data['outflows_on'],
				'model': 'feldmann', #gal_data['outflows_model'],
		...
		gal_data['recycle'] = { 'on': gal_data['recycle_on'],
        	...
        	gal_data['isotopes'] = gal_data['isotopes'].split(b";")
        	gal_data['Pristine_isotope_fractions' ] = [float(stri) for stri in gal_data['Pristine_isotope_fractions'].split(b";")]                           
        	init_list.append(gal_data)
        #runModel(gal_data)
    procs = []

    pool = Pool(processes=20)  # set up parallel processing pool and number of processors to use.
